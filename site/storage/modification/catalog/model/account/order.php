...
//prostore
	public function getOrders899($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
	...
	public function getOrderStatusId($order_status) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status  WHERE name = '" . $this->db->escape($order_status) . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
