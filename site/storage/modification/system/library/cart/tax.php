...
// prostore	
	public function clearTax($value, $tax_class_id) {
		$amount = 0;

		$tax_rates = $this->getClearRates($value, $tax_class_id);
	...
	public function getClearRates($value, $tax_class_id) {
		$tax_rate_data = array();
		...
		if (isset($this->tax_rates[$tax_class_id])) {
			foreach ($this->tax_rates[$tax_class_id] as $tax_rate) {
				...
				if ($tax_rate['type'] == 'F') {
					...
					$amount += ($value - $value / (1+$tax_rate['rate']/100));
				}
