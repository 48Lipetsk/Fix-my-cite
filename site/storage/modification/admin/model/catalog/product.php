...
class ModelCatalogProduct extends Model {
			  ...
			  public function changePrice($product_id, $new_price, $price_type){
				  $this->db->query("UPDATE " . DB_PREFIX . "product SET " . $price_type . " = " . $new_price . " WHERE product_id = " . (int)$product_id);
			  }
	...
	public function addProduct($data) {
		...
		link = '" . $this->db->escape($product_option_value['link'] ?? '') . "'");

		$product_id = $this->db->getLastId();

				if (isset($data['model'])) {
					$this->db->query("UPDATE " . DB_PREFIX . "product SET model = CONCAT (LPAD(manufacturer_id,2,'00'), LPAD(product_id,4,'0000')) WHERE product_id = ' " . (int)$product_id . " ' " );
				}
